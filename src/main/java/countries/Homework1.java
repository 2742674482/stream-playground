...
public class Homework1 {
    ...
    public IntSummaryStatistics streamPipeline6() {
        return countries.stream().filter(o->o.getName().length()>0).map(o->o.getName()).mapToInt(x->1).summaryStatistics();
    }
...
ed by the language code "es").
    ...
    public double streamPipeline12() {
        return countries.stream().mapToInt(country->country.getName().length()).sum()/(double)countries.stream().count();
    }
    ...
    public Optional<Country> streamPipeline14() {
         return Optional.of(countries.stream().filter(country -> country.getArea() != null).max(Comparator.comparing(Country::getArea)).get());
    }
